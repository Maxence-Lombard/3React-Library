...
function SearchBar() {
    ...
    const getQuickSearchItems = () => {
        const items = data.docs.slice(0, 5).map(doc => {
            ...
            const author = doc.author_name ? doc.author_name[0] : "Auteur inconnu";
            const coverUrl = doc.cover_i
                ? `https://covers.openlibrary.org/b/id/${doc.cover_i}-M.jpg`
                : null;
            const workId = doc.key ? doc.key.split("/").pop() : null;
    ...
    return (
        <div className={defaultClass.searchBarContainer}>
            ...
            {showDropdown && itemToQuickSearch && itemToQuickSearch.length > 0 && (
                <ul className={defaultClass.quickSearchDropdown}>
                    {itemToQuickSearch.map(book => (
                        <li
                            key={book.workId}
                            className={defaultClass.quickSearchItem}
                            onMouseDown={() => {
                                navigate(`/book/${book.workId}/details`);
                                setShowDropdown(false);
                            }}
                        >
                            { book.coverUrl ? (
                                <img src={book.coverUrl} alt={book.title} width={30} style={{ marginRight: 8 }} />
                            ) : null }
                            <span>{book.title}</span>
                            <span style={{ color: "#888", marginLeft: 8 }}>{book.author}</span>
                        </li>
                    ))}
                </ul>
            )}
